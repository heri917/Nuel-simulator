...
function toggleReverse(){
 if(Math.abs(state.speed)<0.8){ state.reverse=!state.reverse; state.gear=state.reverse?0:1; reverseBtn.classList.toggle("active",state.reverse); }
}
...
function update(dt){
 ...
 const speedFactor=clamp(Math.abs(state.speed)/14,0,1);
 const turnRate=state.steer*.55*speedFactor*(state.reverse?-1:1);
