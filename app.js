...
renderer.domElement.addEventListener("pointerup",e=>{if(camPointer===e.pointerId){camPointer=null;cameraGestureAxis=null}});
renderer.domElement.addEventListener("pointercancel",e=>{if(camPointer===e.pointerId){camPointer=null;cameraGestureAxis=null}});
let lastTouchDist=0;
...
function update(dt){
 ...
 const turnRate=state.steer*.62*speedFactor*(state.reverse?-1:1);
 ...
 const camY=truck.position.y+8.2+Math.sin(state.camPitch)*state.camDist;
 ...
 const look=truck.position.clone().add(forward.multiplyScalar(7.5));
 look.y+=1.8;
 camera.lookAt(look);
